...
- 給与明細の写真から自動入力（OCR、下記参照）
- 給与登録（支給10項目・控除7項目・勤務情報3項目 + 会社独自の「その他の項目」を好きなだけ追加可能）
...
## 給与明細の写真から自動入力する機能について

給与登録画面の「📷 給与明細の写真から自動入力」から、給与明細の写真を選ぶと、
基本給・残業代・控除額などの数値を自動で読み取ってフォームに入力します。

- 文字認識（OCR）は端末のブラウザの中だけで行われ、写真がどこかに送信されることはありません
- 初回利用時のみ、文字認識に必要なファイル（合計10MB強）を読み込みます。2回目以降はキャッシュされるため速くなります
- 明細書のレイアウトによって読み取り精度は変わります。**読み取った内容は必ず目で確認してから登録してください**（金額を並べて表示している一般的な明細であれば認識しやすいですが、複雑な表形式では項目がずれることがあります）

## 項目を追加・編集したい場合

**コードを触らずに増やしたい場合**：給与登録画面の「支給（その他の項目）」「控除（その他の項目）」から
「＋項目を追加」を押すと、会社独自の手当・控除（待機手当、寮費など）を名前と金額を入力するだけで
好きなだけ追加できます。集計・履歴・CSV出力にも自動的に反映されます。

**アプリの標準項目として追加したい場合**：給与の固定項目（支給・控除・勤務情報）は以下の2ファイルで一元管理されています。
